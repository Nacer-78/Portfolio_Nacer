...
export default function About_us () {
    ...
    const [carMovements, setCarMovements] = useState([]);
    ...
    function setCheckpointRef(el, i) {
        checkpoints.current[i] = el
    }
    ...
    useEffect(() => {
        ...
        function updateMovements() {
            ...
            const car = golf_8.current

            if (!track || !car || checkpoints.current.length === 0) return

            const trackRect = track.getBoundingClientRect()
            const carRect = car.getBoundingClientRect()

            const movements = checkpoints.current.map((checkpoint) => {

                if (!checkpoint) return 0

                const checkpointRect = checkpoint.getBoundingClientRect()
                const checkpointCenter = checkpointRect.left + checkpointRect.width / 2
                const targetRelative = checkpointCenter - trackRect.left
                let translate = targetRelative - (carRect.width / 2)
                const maxTranslate = Math.max(0, trackRect.width - carRect.width)

                if (translate < 0) translate = 0
                if (translate > maxTranslate) translate = maxTranslate
                return Math.round(translate)

            })

            const positions = [0, ...movements]
            setCarMovements(positions)
        ...
        function waitAndUpdate() {
            ...
            setTimeout(() => {
                requestAnimationFrame(updateMovements)
            }, 50);

        }

        window.addEventListener('resize', waitAndUpdate)
        waitAndUpdate()

        return () => {
            window.removeEventListener('resize', waitAndUpdate)
        }
    ...
    useEffect(() => {
        ...
        currentIndexRef.current = numberClicks

    }, [numberClicks]);

    useEffect(() => {

        if (carMovements && carMovements.length > 0) {
            showDiv(currentIndexRef.current || 0)
        }
    ...
    function showDiv(indexToShow) {
        ...
        all_div_hidden.current.forEach((div_hidden, i) => {
            if (div_hidden) div_hidden.classList.toggle('hidden', i !== indexToShow)
        })

        let distance

        if (carMovements && carMovements.length > 0) {
            distance = carMovements[indexToShow] ?? carMovements[carMovements.length - 1]
        } else {
            distance = 0
        }
        ...
        if (golf_8.current) {
            ...
            golf_8.current.style.transition = "transform 800ms ease"
        ...
        checkpoints.current.forEach((checkpoint, i) => {
            ...
            if (checkpoint) checkpoint.style.visibility = i < indexToShow ? 'hidden' : 'visible'

        })

        checkpoints.current.forEach(cp => cp?.classList.remove("checkpoints_golf_animate"))

        if (checkpoints.current[indexToShow]) {
            checkpoints.current[indexToShow].classList.add("checkpoints_golf_animate")
        }

        currentIndexRef.current = indexToShow
    ...
    function checkPoints(e) {
        ...
        const maxIndex = all_div_hidden.current.length - 1
        ...
        <div className="global_container p-[50px] max-[426px]:p-[25px]">
            ...
            {contents_div_hidden.map((content, index) => (
                ...
                <div key={index} className="hidden" ref={div_hidden => (all_div_hidden.current[index] = div_hidden)}>
                    ...
                    <p className="mb-[50px] whitespace-pre-line paragraph_text">
                ...
                <div ref={trackRef} className="relative w-full">
                    ...
                    <div className="grid grid-cols-3 gap-4 items-end ">
                        ...
                        <div className="col-span-1 relative">
                            <img src={Golf_8} ref={golf_8} className="golf_8" alt="Golf"/>
                        </div>

                        <div className="col-span-2 mb-[5px]">

                            <div className="flex justify-between items-end text-[60px] checkpoints_row">

                                {[0, 1, 2].map((_, i) => (

                                    <div className="flex justify-end" key={i} style={{ width: "33%" }}>
                                        <p ref={el => setCheckpointRef(el, i)} className="checkpoints_golf_animate" onClick={() => { setNumberClicks(i); currentIndexRef.current = i; }}>
            ...
            <div className="mt-[50px] grid grid-cols-2 gap-10 ">
                ...
                <div className="flex justify-end">
                    <button className="brake_pedal_button" onClick={checkPoints_reverse}>
                        ...
                        <img src={Brake_pedal} className="w-[150px] h-auto pedal_icons" alt="Brake pedal"/>
                        <img src={Brake_pedal_hover} className="w-[150px] h-auto pedal_icons_hover" alt="Brake pedal"/>
                        </div>
                    </button>
                </div>
                <div className="flex justify-start">
                    <button className="accelerator_pedal_button" onClick={checkPoints}>
                        <div className="Image_hover">
                            <img src={Accelerator_pedal} className="w-[150px] h-auto pedal_icons" alt="Accelerator pedal"/>
                            <img src={Accelerator_pedal_hover} className="w-[150px] h-auto pedal_icons_hover" alt="Accelerator pedal"/>
                        </div>
                    </button>
                </div>
